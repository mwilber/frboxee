...
# Logging.
def log(s):
    Foundation.NSLog( "FRBoxee: %s" % str(s) )
...
class RUIPythonAppliance( BRAppliance ):
    ...
    def applianceController(self):
        return BoxeePlugin.alloc().init()


class BoxeePlugin(BRController):
    ...
    def wasPushed(self):
        ...
        while not found:
            ...
            # check the list of launched app to see ifBoxee has finished loading.
    ...
    def tick_(self, senders):
        ...
        # Check to see ifBoxee is running.
        found = False
        for app in NSWorkspace.sharedWorkspace().launchedApplications():
            if app['NSApplicationName'] == "BOXEE":
        ...
        # If we don't findBoxee running, then we exited. So bring FR back.
        if not found:
            frController = BRAppManager.sharedApplication().delegate()
